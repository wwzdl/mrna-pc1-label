...
2. **高影响 study 先由 reference-free 稳定性筛查。** Leave-one-study-out PC1 stability 在不使用 Saluki 标签或下游模型分数时将 `Gejman` 排为影响最大的 human study；Saluki agreement 与 ortholog concordance 随后验证其后果。
...
5. **0.10 弱同源正则化是有边界的核心标签结果。** 该 target 与 human no-Gejman PC1 的 `r=0.998`、`RMSE=0.065`；cross-target evaluation 的 Pearson 变化为 `+0.0003`（95% CI `-0.0006` 至 `0.0012`），说明它保留了原 human 标签的可预测结构，但不能进入固定 Saluki human PC1 的同一排行榜。
...
- 固定目标 benchmark、cross-species transfer 和 ortholog-regularized target 应分开表述。
...
- `weak ortholog regularization with cross-target validation`
