...
2. **高影响 study 先由 Saluki-label-independent 稳定性筛查。** Leave-one-study-out PC1 stability 在不使用 Saluki 标签或下游模型分数时将 Gejman 排为 primary sample-weighted analysis 的第一位；条件性同样本量删除与 study balancing 限定几何解释，Saluki agreement 与 ortholog concordance 随后检查改变方向。
...
5. **0.10 弱同源正则化是有边界的核心标签结果。** 在 12,307 个 model-eligible prediction genes 上，该 target 与 human no-Gejman PC1 的 $r=0.9982$；在单独定义的 10,768 个 mapped one-to-one ortholog pairs 上，shift RMSE 为 0.065。Cross-target evaluation 的 Pearson 变化为 +0.0003（95% CI -0.0006 至 0.0012），未检出原 human 标签可预测性下降；严格 direct-vector 消融显示精确 10% mouse 向量仅贡献 0.0013 的相关增量。
