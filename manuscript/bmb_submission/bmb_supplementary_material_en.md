# Supplementary Information: Study-aware label auditing, weak ortholog regularization, and cross-species transfer for mammalian mRNA half-life prediction

**Journal:** *Bulletin of Mathematical Biology*  
**Article title:** Study-aware label auditing, weak ortholog regularization, and cross-species transfer for mammalian mRNA half-life prediction  
...
This Supplementary Information supports the three claims in the main text. First, study-aware label auditing uses reference-free PC1 stability to screen influential studies and then validates their consequences through Saluki agreement and ortholog concordance. Second, with Saluki human PC1 fixed, the human-only baseline and cross-species transfer setting are evaluated separately and supported by permutation, paired bootstrap, residual decomposition, and multi-seed checks. Third, weak ortholog regularization at 0.10 defines a human-dominant mammalian stability target and is bounded by label distance, study-noise scale, cross-target evaluation, and prior ablation. This target is a core label-construction result, but it does not enter the fixed Saluki human PC1 ranking.

Compared with the main text, the purpose of the Supplementary Information is not to repeat the highest score, but to preserve the methodological details required to support the conclusions, including sample-level PCA definitions, the complete study-influence result paths, tuning robustness, prior-permutation and residual controls, ortholog label distances, training-universe boundaries, 10-fold split-sensitivity checks, and implementation notes (Agarwal and Kelley 2022; Chen and Guestrin 2016; Pedregosa et al. 2011).

## S2. Data extraction, preprocessing, and ortholog definitions

This section expands the fields, parameters, and definitions only briefly described in Section 3.1 of the main text. The source data are still taken from the public supplement of the Saluki study, specifically the `MOESM2` and `MOESM3` tables (Agarwal and Kelley 2022). `MOESM2` contains transformed human and mouse mRNA half-life gene by sample matrices. The first two columns are the Ensembl gene ID and gene name, and the remaining columns are transformed values from different studies, experimental methods, cell types, and biological replicates. This table is the primary input for label reconstruction and leave-one-study-out analysis in the present work. `MOESM3` is the more heavily processed matrix released with the Saluki paper. Its third column is the published `half-life (PC1)` summary label. In this manuscript, the human-side label is referred to as Saluki human PC1 and the mouse-side label as the Saluki mouse prior; the remaining columns are processed sample values. In prior-related descriptions, the notation can therefore be summarized as follows: `mouse_pc1` denotes our locally reconstructed mouse prior, whereas the second mouse-side prior is the label released by Saluki. The human worksheet of `MOESM3` indicates that this PC1 was derived after removal of `Gejman`, whereas the mouse worksheet uses the full mouse dataset. Accordingly, `MOESM3` serves mainly as a reference and control resource here rather than as a replacement training-label source for the main reconstruction workflow. Because `MOESM2` already contains transformed values, we do not apply an additional log transform and instead begin with sample-wise z-score, followed by iterative PCA missing-value imputation and quantile normalization (Tipping and Bishop 1999; Troyanskaya et al. 2001; Bolstad et al. 2003).
...
| Saluki-like coverage | human `min_observed_per_gene=10`; mouse `min_observed_per_gene=5` | locally reconstructed human/mouse labels under a Saluki-like coverage rule | Used for 0.10 weak ortholog regularization |
...
| Study influence | full rerun of the above pipeline after leave-one-study-out removal | PC1-stability screen and Saluki-agreement validation | Reference-free stability screens studies; Saluki agreement validates; downstream scores are not used |
...
| 0.10 weak ortholog regularization | human no-Gejman PC1 + mapped mouse PC1 + `λ` | Not a standalone predictor for arbitrary new sequences | human-dominant mammalian stability target | Should not be presented as direct human half-life or ranked with the fixed target |
...
The main text already presents the human leave-one-study-out result in Fig. 4, so we retain only the full mouse ranking here. Complete human and mouse values are stored in `results/study_influence/human/study_influence.tsv` and `results/study_influence/mouse/study_influence.tsv`, respectively. The human audit has two stages. `PC1 stability` does not use Saluki PC1 and ranks `Gejman` as the most influential study among all candidates. `Saluki-PC1 agreement gain` then provides reference-informed validation, and ortholog concordance supplies an orthogonal cross-species validation. No human-only, prior-enhanced, or ortholog-regularized model score entered this assessment. In mouse, removal of `Hanna` changes full-label geometry but does not improve agreement with the Saluki mouse prior, so it is not interpreted as a mouse analogue of the human case.

This human/mouse asymmetry shows that high influence does not automatically justify removal. Our conclusion is that the audit recovers and quantifies the dominant influence of `Gejman` on the present consensus geometry, not that the study is invalid for every biological analysis. The mouse compendium lacks the same convergence of evidence and is therefore kept intact.

![](figures/supplement/FigS_mouse_study_influence.png)

**Supplementary Fig. S3** Mouse leave-one-study-out full ranking. a, Correlation between each leave-one-study-out PC1 and the full mouse PC1; lower values indicate greater influence on consensus-label geometry. b, Change in agreement with the Saluki mouse prior after removing each study. `Hanna` has the lowest stability but a negative agreement change, so mouse does not show the same convergence of evidence observed for human `Gejman` in main-text Fig. 4

## S5. Tuning scan and supplementary robustness notes

This section retains only one necessary engineering supplement: under the current unified XGBoost setting, the performance of human-only `compact_all` is not sensitive to small parameter perturbations, so the human baseline reported in the main text does not depend on a single accidental “best” parameter choice. The stability of the prior-enhanced results is instead supported by the shuffled control, 10-fold robustness, and residual analysis in the main text and later sections of this supplement, without relying on additional linear-weighting plots or repeated subset displays.

![](figures/supplement/FigS_tuning_scan.png)

**Supplementary Fig. S4** Compact-all tuning scan. Horizontal bars rank ten XGBoost parameter configurations by Pearson correlation; the orange bar marks the fixed main-setting configuration. Bar lengths use unrounded Pearson correlations, and end labels show five decimal places. Performance ranges only from 0.735 to 0.741, indicating that the human-only compact baseline is not determined by a single narrowly optimized setting
...
### Supplementary Table S4. Two-stage OOF decomposition in prior residual analysis
...
## S8. Additional interpretation and cross-target evaluation of 0.10 weak ortholog regularization

This analysis changes the **supervised target**, whereas the global prior benchmark changes **input information**; the two settings must remain separate. The 0.10 ortholog-regularized target combines 90% human no-Gejman z-scored label with 10% mapped reconstructed-mouse z-scored label, while genes without an ortholog signal retain the human label. It is not a direct experimental half-life value, but a human-dominant mammalian stability score derived from a multi-study compendium.

Label geometry supports human dominance. Before regularization, human no-Gejman PC1 and locally reconstructed mouse PC1 have Pearson = 0.789, RMSE = 0.663, and MAE = 0.512 across 10,768 one-to-one orthologs. After regularization, the target correlates with mouse PC1 at 0.827 but remains correlated with human no-Gejman PC1 at 0.9982 (95% bootstrap CI, 0.9981-0.9983), with RMSE = 0.065 and MAE = 0.050. Its displacement is also much smaller than ordinary study-to-study differences (Section S9 and Supplementary Table S12).
...
Cross-target evaluation directly tests whether changing the target harms the original human label. Using the same 12,307 genes, 1802 human-only features, 10 folds by 3 seeds, and identical model parameters, training on human no-Gejman PC1 gives r = 0.7476 ± 0.0010 against that target. Training on the regularized target and still evaluating against the original human target gives r = 0.7480 ± 0.0010. The paired difference after combining the three OOF predictions is +0.0003 (95% CI, -0.0006 to 0.0012). No detectable loss or artificial gain in human-label predictability is observed at the present resolution.

With explicit mouse priors, prediction of the regularized target reaches 0.855 ± 0.001, whereas the fixed Saluki human PC1 prior-enhanced baseline on the same universe is 0.839 ± 0.001. This difference reflects stronger alignment with conserved ortholog covariates, not improved pure human sequence-only capacity. Removing the locally reconstructed mouse PC1 used to construct the target, while retaining the Saluki mouse prior and indicators, still gives 0.852 ± 0.001; retaining only missingness and high-confidence indicators returns performance to 0.753 ± 0.001. The 0.10 target is therefore a bounded core label-construction result, not a replacement ranking for the fixed human target.

## S9. Scale comparison between the `λ = 0.10` label shift and study-level noise

The main text provides the corresponding evidence in Fig. 6c and Table 3 that the label shift induced by regularization is much smaller than study-level noise. This section records the exact calculation protocol. Two concepts must first be separated. `human no-Gejman PC1` is a consensus label integrated across many studies, whereas a `study mean label` is a single-study proxy obtained by averaging the samples within one study. The latter naturally carries much stronger study-specific measurement noise, so pairwise correlations between study mean labels should not be misread as the stability of the final consensus label.
...
The third tier is cross-species prior-enhanced prediction for human genes with mappable mouse ortholog priors. Inputs include the two prior values (`mouse_pc1` and Saluki mouse prior) and their availability/high-confidence indicators. These are mouse-side covariates external to the human target. Unlike 0.10 ortholog regularization, this setting does **not** change the supervised target, which remains Saluki human PC1. Genes without either mouse prior should use coverage-aware fallback to the human-only compact model.
...
### Supplementary Table S5. Training universes, prior-availability subsets, and label-comparison sets
...
| `both-priors` subset | 11,107 genes | Genes in the global prediction universe with both `mouse_pc1` and the Saluki mouse prior available | Prior-coverage interpretation, prior-only/residual decomposition, and prior-complete stratified analysis |
...
| 0.10 ortholog-regularized universe | 12,307 genes | Separate human gene universe used for weak ortholog regularization | Evaluation of the 90:10 regularized target |
| one-to-one ortholog label-distance set | 10,768 pairs | Pairs with human label, mouse label, and Ensembl one-to-one ortholog mapping | Correlation and error comparison between the regularized target and human/mouse labels |

## S11. Control experiments with MOESM3-derived supervision labels

The `MOESM3` control experiments address a very specific question: because the Saluki supplement already provides a processed matrix and a `half-life (PC1)` summary label, should we replace Saluki human PC1 with labels re-derived from `MOESM3` itself as the main supervision target? Supplementary Table S6 shows that the answer is no. When all human `MOESM3` samples, including `Gejman`, are retained, the ability of the derived labels to recover Saluki human PC1 declines overall. After removing `Gejman`, the `as_is_pc1` label essentially returns only to a level equivalent to Saluki human PC1. Only `no_gejman_rerun_pipeline_pc1` gives a very small human-only gain, on the order of about `+0.001`.
...
### Supplementary Table S6. Downstream benchmark for `MOESM3`-derived supervision labels
...
### Supplementary Table S7. Focused benchmark of `MOESM3`-derived supervision labels under prior settings
...
## S13. Supplementary tables for 10-fold robustness, weak ortholog regularization, and uncertainty

Supplementary Tables S8-S14 collect the strict checks. S8 reports 10-fold robustness for the global prior benchmark. S9 compares the 0.10 ortholog-regularized target with the fixed Saluki-PC1 baseline on the same universe. S10 reports prior ablations. S11-S12 report label distance and study-noise scale. S13 reports human-only cross-target evaluation. S14 provides paired-bootstrap CIs for label auditing, ortholog validation, and prior gain.

### Supplementary Table S8. 10-fold robustness of the global prior benchmark
...
### Supplementary Table S9. 10-fold target benchmark for 0.10 weak ortholog regularization
...
### Supplementary Table S10. 10-fold prior-ablation benchmark for 0.10 weak ortholog regularization
...
### Supplementary Table S11. Human-mouse ortholog label distance
...
### Supplementary Table S12. Study-level noise versus `λ = 0.10` label shift
...
### Supplementary Table S13. Human-only cross-target evaluation of the 0.10 ortholog-regularized target
...
Against the original human no-Gejman PC1, averaging the three OOF predictions for each gene gave Pearson = 0.75233 after human-target training and 0.75259 after regularized-target training. This averaging summarizes split sensitivity and is not an additional model-blending step. The paired difference was +0.00026, with a 95% CI of -0.00061 to 0.00118 from 2,000 gene bootstraps. Because the interval crosses zero, the analysis provides no evidence that regularized-target training either reduces or improves predictability of the original human label at the present resolution; it is not presented as a formal equivalence test with a prespecified margin.

### Supplementary Table S14. Paired-bootstrap uncertainty for key correlation differences
...
Both the main text and the Supplementary Information preserve four interpretation rules. First, the pure human route is a transparent human-only fixed-target benchmark. Second, higher fixed-target performance is limited to the cross-species transfer setting because the inputs explicitly contain mouse ortholog priors. Third, 0.10 weak ortholog regularization is a core label-construction result, but 0.855 describes predictability of the regularized target from explicit cross-species inputs. Cross-target evaluation supports preservation of human-label predictability; it does not turn the result into a fixed Saluki human PC1 or pure-sequence ranking. Fourth, the framework is not a one-click predictor for arbitrary unannotated sequences: complete compact/prior-enhanced prediction depends on precomputed regulatory features and mouse ortholog priors, whereas a new sequence can enter only the base-sequence tier after 5'UTR / CDS / 3'UTR annotation.
...
### Supplementary Table S15. Main reproduction entry points and result files
...
| 0.10 weak ortholog regularization | `results/ortholog_regularized_label_10fold_main/summary_by_label.tsv`, `results/ortholog_regularized_label_multiseed/summary_by_label.tsv`, `results/ortholog_prior_ablation_lambda0p1_10fold/summary_by_setting.tsv` | Records the `λ = 0.10` target benchmark, λ sensitivity, and prior ablations |
| Human-only cross-target evaluation | `results/ortholog_regularized_cross_target/summary_aggregate.tsv`, `results/ortholog_regularized_cross_target/paired_bootstrap_ci.tsv` | Tests whether regularized-target training preserves original-human-label predictability |
...
| Main-text weak-regularization validation figure | `scripts/plot_bmb_ortholog_regularized_target.py`, `manuscript/bmb_submission/figures/main/Fig06_ortholog_regularized_target_cn.*` | Generates the label-geometry, study-noise, and cross-target panels in Fig. 6 |
| Main-text ortholog validation figure | `scripts/redraw_bmb_ortholog_combined_cn.py`, `manuscript/bmb_submission/figures/main/Fig05_ortholog_summary_cn.*` | Generates the paired scatter, correlation-summary, and gain panels in Fig. 5 directly from the ortholog result tables |
| Label auditing and ortholog validation | `results/study_influence/**/*.tsv`, `results/ortholog_analysis/*.tsv` | Stores leave-one-study-out, Saluki-label agreement, and human-mouse consistency results |
