...
**作者：** Wenzhuo Wang，Ying Shao<br>
**单位：** 大连海事大学理学院，辽宁大连 116026<br>
...
| --- | --- | ---: | --- |
| 标签重建/审计 | L1，审计阈值 human full reconstruction（≥3 个观测） | 16,444 genes | Full-human PC1、sample PCA 和 human LOO 的起始标签 |
| 标签重建/审计 | L2，审计阈值 mouse full reconstruction（≥3 个观测） | 16,951 genes | Mouse PC1、mouse PCA/LOO 和 fixed-target 重建 mouse-prior 来源 |
| 标签重建/审计 | L3，审计阈值 human no-Gejman reconstruction（≥3 个观测） | 15,788 genes | No-Gejman PC1 和 primary dynamic-coverage Gejman comparison |
...
| Target shrinkage | S1，目标构建 human no-Gejman universe（human ≥10；mouse comparator ≥5） | 12,644 genes | 构建 0.10 ortholog-informed target |
...
本地流程提供一个可审计、可反复运行的标签重建，同时承认 Saluki 未公开的处理细节无法被精确复制。为避免覆盖度不同的 PC1 被误认为同一向量，本文采用五个明确名称：**审计阈值 human PC1**、**审计阈值 human no-Gejman PC1** 和 **审计阈值重建 mouse PC1** 均要求每个基因至少 3 个样本有观测；**目标构建 human no-Gejman PC1** 与 **目标构建重建 mouse PC1** 则分别要求 human 至少 10 个、mouse 至少 5 个样本有观测。它们均由 MOESM2 本地重建；“Saluki-like” 仅指 10/5 coverage thresholds。在审计阈值 full-human 标签与 Saluki human PC1 共同覆盖的 13,921 个基因上，Pearson 为 0.948。审计阈值重建 mouse PC1 与 Saluki mouse PC1 在共同基因上的 Pearson 为 0.958。后文用于 Gejman 配对推断的 full-label Pearson 0.9515 来自另行固定的 A2 集合（13,265 个同时具有 full、no-Gejman 和 Saluki 标签的基因）。
...
由于 Gejman 占 human 全部 54 个样本中的 15 个，我们增加了条件性同样本量删除分析。每次从其余 39 个非 Gejman 样本中无放回抽取 15 个样本，完整重跑标签流程，共重复 $B=500$ 次，因此每个比较数据集都保留 Gejman。令 $N_{\mathrm{extreme}}$ 表示条件性比较分布中至少与观测统计量同样极端的次数，单侧经验尾部比例定义为
...
我们用少量 ortholog constraint 构建反映哺乳动物保守稳定性的 human-dominant target。该步骤改变监督目标，但保持 human 特征定义不变。我们分别构建目标构建 human no-Gejman PC1（至少 10 个观测）和目标构建重建 mouse PC1（至少 5 个观测），并各自执行 z-score；随后通过 one-to-one ortholog 将 mouse label 映射到 human genes。对有 ortholog signal 的 human gene，令 $H$ 表示标准化 human label，$M$ 表示映射后的标准化 mouse label。一般收缩目标及本文采用的操作点为
...
Human-only 路线保持目标不变，并使用两层特征空间。第一层为 526 维 base sequence features，由 Ensembl release 115 的 representative transcripts 生成，覆盖 5′UTR、CDS 和 3′UTR 的长度、GC content、codon frequency、region-level 3-mer 以及 3′UTR 4-mer。本文将这 526 个序列特征与 Saluki datapack 中预计算的 319 个 CWCS、780 个 SeqWeaver 和 177 个 DeepRiPe 特征合并，定义为 1,802 维 compact sequence/regulatory feature set，并记为 `compact_all`(Agarwal et al. 2015; Park et al. 2021; Ghanbari and Ohler 2020)。因此，`compact_all` 是本文对该组合特征集的命名，不是 Saluki 原文中的模型名称。

所有 human-only benchmark 均以 Saluki human PC1 为主监督目标，并采用 gene-level out-of-fold evaluation。我们先在同一 P2 compendium 上对 5 组 base-sequence XGBoost 设置进行探索性扫描，以 OOF Pearson 最高为选择标准，$R^2$ 仅用于同分时判定；随后将选定参数冻结，用于 compact、transfer 及其匹配控制：`tree_method=hist`、`device=cuda`、`max_depth=6`、`learning_rate=0.02`、`min_child_weight=4`、`subsample=0.9`、`colsample_bytree=0.75` 和 `reg_lambda=1.0`。该选择未嵌套于独立外部评估，因此绝对 OOF 分数属于描述性 benchmark estimate；real-prior 与 shuffled-prior 比较仍使用相同冻结参数和 folds。Human-only benchmark 最多允许 5,000 棵树并使用 120 轮 early stopping；global prior benchmark 最多允许 3,000 棵树并使用 100 轮 early stopping。在每个 outer fold 内，从 outer-training genes 中再保留 `max(512, outer-training genes 的 10%)` 作为 inner validation subset。外层 held-out fold 不参与 early stopping，只在模型拟合完成后用于一次最终预测。主文报告 5-fold OOF 结果，并用 10-fold × 3 random seeds 作为 split-sensitivity robustness check。扫描脚本、完整结果表和机器可读选择记录见补充材料 S15。
...
本文用四类控制回答四个直接问题。第一，permutation control 检查 prior-enhanced gain 是否依赖 gene-specific prior，而不只是多出几列特征或特定缺失模式：在每个 outer fold 内，分别在 fit、inner-validation 和 held-out 分区中打乱 `mouse_pc1` 和 Saluki mouse PC1，且只在原本具有对应 prior 数值的 genes 之间进行；prior 可用性与 high-confidence 指示列保持不变。第二，residual analysis 检查 transfer result 是否只是在复述 prior：分析限定在同时具有两类 mouse priors 的 `both_priors_available` 子集（N=11,107）。在每个 outer fold 内，对 outer-training genes 执行 5-fold inner cross-fitting，得到每个训练基因的 RidgeCV nuisance-prior OOF prediction，并据此定义 residual target；另用全部 outer-training genes 拟合 RidgeCV，预测 outer held-out fold。XGBoost 仅从 outer-training human `compact_all` features 学习 residual，因此最终 held-out prediction 的两个组成部分均相对 outer fold 保持 OOF。第三，主要 shrinkage-target predictor 使用审计阈值重建 mouse PC1（至少 3 个观测）与 Saluki mouse PC1。更严格的成对消融先把本地 prior 换成构造 target 时使用的精确目标构建 mouse PC1（至少 5 个观测），随后只移除这条精确数值，同时保留 Saluki mouse PC1 及全部指示列。该比较专门隔离模型直接重复读取构成 target 的 10% mouse 向量这一可能性。第四，3.6 节的 cross-target evaluation 检查目标收缩是否损害原 human label 上的折外预测。

模型性能报告 Pearson、Spearman 和 $R^2$；标签距离报告 Pearson、Spearman、MSE、RMSE、MAE、median absolute error 和 p95 absolute error。除非特别说明，所有 prediction results 都是 out-of-fold predictions，而不是训练集内拟合分数。mean ± SD 表示不同 random seeds 对数据划分的敏感性，不作为抽样置信区间。需要每个 gene 只有一个 prediction 的 paired bootstrap 先对三组 seed-specific OOF predictions 求均值，形成 **seed-averaged OOF ensemble**；其区间以已拟合的 seed ensemble 和基因重采样为条件，不包括重新训练或重新划分 folds 的不确定性。Cross-target comparison 使用 2,000 次重采样，其余主张使用 5,000 次重采样（补充表 S14-S15）。主文报告支撑核心结论的结果，补充材料保留完整 ranking、multi-seed summaries、prior ablation、label distance、study-noise comparison 与 bootstrap 输出。
...
条件性同样本量分析改变了这一几何排序的解释。500 次完整流程重跑中，每次从 39 个非 Gejman 样本随机移除 15 个并保留 Gejman，PC1 stability 中位数为 0.944（比较分布中央 95% 范围 0.932-0.956）；Gejman 的 0.955 并非异常偏低（经验低尾比例=0.964）。在 equal-study PCA weighting 和 study-mean collapse 下，Gejman 也均由第 1 降至第 3。其 raw PC1 geometry 影响因此部分来自样本数，现有数据未将其识别为 sample-count-adjusted statistical outlier。

但标签改变的方向具有 study specificity。在固定的 13,265 个配对共同基因上，移除 Gejman 后与 Saluki human PC1 的 Pearson 由 0.9515 升至 0.9829，增益 0.0314（95% bootstrap 置信区间 0.0298-0.0330）。500 次从非 Gejman 样本中删除 15 个样本的条件性比较均未获得同等增益（比较分布中位数 -0.0926；经验高尾比例=1/501=0.002）。图 4b 显示的是 primary dynamic-coverage 口径，每次移除后在当时可用的配对基因上计算；Gejman 的该估计为 +0.0349。该处理参照比较不参与 stability 排序，任何下游模型分数也未进入 study audit；由于 Saluki human PC1 本身是在移除 Gejman 后发布的，该结果量化的是对已发布处理选择的恢复，而不是独立验证。上述条件性尾部比例只描述删除敏感性，不是经 study 选择校正的 whole-study 显著性检验。

![](figures/main/Fig04_human_study_influence.png)

**图 4** Human studies 的 primary sample-weighted leave-one-study-out audit。a，点图显示对 full-label geometry 影响最大的 8 个 study；横坐标为 PC1 stability，数值越低表示影响越大，虚线表示 $r=1$。该排序不使用 Saluki PC1。b，agreement change 定义为 Pearson 相关差 $\Delta r=r_{\mathrm{LOO,Saluki}}-r_{\mathrm{full,Saluki}}$，并在每次移除后可用的配对基因上按 primary dynamic-coverage 口径计算。Gejman 的图中数值为 +0.0349；正文用于 bootstrap 的固定 13,265-gene 配对估计为 +0.0314。条件性同样本量、fixed-universe、预处理与 study-balanced 分析见补充材料 S5 和补充图 S4。

Human-mouse ortholog 提供第二项比较。在 12,592 个 one-to-one pairs 中，移除 Gejman 后 Pearson concordance 由 0.741 升至 0.755，增益 0.0147（95% bootstrap 置信区间 0.0111-0.0181）；Spearman 由 0.730 升至 0.744。Ensembl high-confidence subset 中的增益仍为 0.0146。500 次条件性删除均未达到该 full-set ortholog gain（比较分布中位数 -0.0616；经验高尾比例=1/501=0.002）。样本数影响标签移动幅度；移除 Gejman 则使标签同时靠近 Saluki 处理参照和保守 ortholog 结构。
...
Permutation control 将增益定位到 gene-prior correspondence。Primary 5-fold 分析中，real dual-prior model 达到 Pearson=0.829、R2=0.688；在每个 outer fold 内分别按 fit、inner-validation 和 held-out 分区打乱 prior 后，Pearson 回到 0.745、R2 回到 0.553，接近 human-only baseline。Shuffling 保持 prior availability 与 high-confidence indicators 不变，仅破坏每个 gene 与 prior value 的对应。在 seed-averaged OOF ensemble 中，real-prior 与 shuffled-prior 的相关分别为 0.8324 和 0.7531，配对增益 0.0794（95% 置信区间 0.0735-0.0855）。该区间以已拟合的三 seed ensemble 和基因重采样为条件。

Residual decomposition 表明 prior 很强，但并非全部信息。在同时具有两类 priors 的 11,107 个 genes 上，prior-only RidgeCV 的 Pearson=0.792、$R^2=0.627$，human `compact_all` alone 的 Pearson=0.739、$R^2=0.544$。加入一个从 inner-cross-fitted outer-training residual 学得的 OOF human-feature correction 后，最终达到 Pearson=0.826、$R^2=0.675$。Human features 对 prior 未解释方差的解释比例定义为
...
式（9）的结果为 12.9%。该子集分析用于分解信息，不替代 12,916-gene 主 benchmark。

### 4.4 弱同源基因目标收缩保持 human 主导性，未检测到 human-label 可预测性下降

该分析改变监督 target。在 12,307 个 model-eligible genes 上，收缩 target 与目标构建 human no-Gejman PC1 的 Pearson 为 0.9982（95% bootstrap 置信区间 0.9981-0.9983）。在单独定义的 10,768 个 mapped one-to-one ortholog pairs 中，收缩前目标构建 human no-Gejman PC1 与目标构建重建 mouse PC1 的 Pearson 为 0.789（RMSE=0.663；MAE=0.512）；应用 $\lambda=0.10$ 后，新 target 与 human label 的 Pearson 为 0.9979（RMSE=0.065；MAE=0.050），与 mouse PC1 的 Pearson 为 0.827。该操作因此只对 human label 作轻度 ortholog-informed 调整。它属于目标构建，不向 prediction model 添加 penalty。
...
| --- | ---: | ---: | ---: | ---: | --- |
| Shrinkage target vs 目标构建 human no-Gejman PC1 | 10,768 ortholog pairs | 0.998 | 0.065 | 0.050 | human 标签结构仍占主导 |
| Shrinkage target vs 目标构建 mouse PC1 | 10,768 ortholog pairs | 0.827 | 0.601 | 0.464 | 仅向跨物种保守方向轻微移动 |
| 目标构建 human no-Gejman PC1 vs mouse PC1 | 10,768 ortholog pairs | 0.789 | 0.663 | 0.512 | 收缩前 human 与 mouse 标签已存在差异 |
...
**图 8** 0.10 ortholog-informed target shrinkage 保持 human 主导结构，且未检测到原 human 标签可预测性下降。a，收缩 target 与目标构建 human no-Gejman PC1 几乎沿对角线分布。b，该 target 与目标构建重建 mouse PC1 的相关低于其与 human label 的相关。c，其位移小于同一 z-scored scale 上的 study-study 和 study-consensus 差异。d，两类 target 使用相同 12,307 genes、human-only features、folds、seeds 和参数训练，并统一在原 human target 上评估。点为 seed-averaged OOF ensemble 的 Pearson 差，误差线为条件性 gene-level paired bootstrap 95% 置信区间；正文中的数值为 across-seed mean±SD。

对该 target 的预测构成独立 conserved-target test。它与 Saluki human PC1 的相关仍为 0.987。在同一 12,307 genes 上，human-only prediction 为 0.7538±0.0010。主要输入把审计阈值重建 mouse PC1 与 Saluki mouse PC1 同时作为 prior，得到 0.8551±0.0007。更严格的成对敏感性分析改用与 target 构建完全相同的目标构建 mouse PC1，并与 Saluki mouse PC1 联合输入，结果为 0.8537±0.0004；只移除这条精确 mouse 向量、保留 Saluki mouse PC1 及全部 indicators 后仍为 0.8524±0.0005，二者相差 0.0013。仅保留 indicators、不输入 mouse 数值时为 0.7538±0.0007。因此，模型直接重复读取构成 target 的 10% mouse 向量只解释了 transfer result 的一小部分，但较高性能仍依赖外部跨物种协变量。由于这里使用不同 target，该结果与 fixed Saluki human PC1 下的模型排名分开报告。
...
本文仍有四方面局限。第一，标签审计和主要 benchmarks 仍基于单一公开 compendium。条件性同样本量分析从非 Gejman 样本中删除 15 个个体样本，每个比较数据集均保留 Gejman；因此它以已选 study 为条件，既不模拟完整相关 pseudo-study 的删除，也未校正 study 选择。Study balancing 也改变了几何排序，仍需独立 half-life 资源检验可迁移性。第二，mouse priors 虽独立于 human target 构建，却不是每个稳定性决定因素在机制上完全独立的测量。Cross-species transfer 与 0.10 target shrinkage 分别改变输入信息和 target definition，不能放在同一 leaderboard；经验选择的 $\lambda=0.10$ 也需独立校准。第三，cross-validation 按单个 gene 随机分折，没有按 paralog family 分组；gene 和 ortholog-pair bootstrap 也以 genes 或 pairs 为独立重采样单位。因此，相关基因可能同时出现在 fold 两侧，pathway 或 paralog family 内依赖可能使 OOF 性能偏乐观或使条件性区间偏窄。第四，模型参数来自同一 compendium 上的探索性扫描，而非嵌套的外部 benchmark；本文也没有重新训练 Saluki 或其他 large sequence models。当前实现不是 arbitrary unannotated sequences 的 one-click predictor，因为 base-sequence、compact-regulatory 与 prior-enhanced predictions 需要不同输入资源。

## 7. 结论

Study-aware auditing 表明，Gejman 对 human consensus-label geometry 的影响具有样本数敏感性，但其 Saluki agreement 与 ortholog concordance 的正向增益未被 500 次删除 15 个非 Gejman 样本的条件性比较复制。在固定 Saluki human PC1 目标下，compact human-only model 提供了可复现基线；加入 gene-specific mouse priors 后，重复 10-fold 性能升至 Pearson=0.830，而 permutation 会消除该增益。$\lambda=0.10$ 的 ortholog-informed target shrinkage 仍与 human no-Gejman PC1 几乎一致，cross-target evaluation 未检测到其折外可预测性下降。这些结果构成一套可审计的 mammalian mRNA half-life benchmark 框架。
...
代码、图件生成脚本和结果复现流程已通过 [wwzdl/mrna-pc1-label](https://github.com/wwzdl/mrna-pc1-label) 公开；与当前两作者稿件对应的审计版本标记为 `mRNA-PC1-label-v1.4.5`。
