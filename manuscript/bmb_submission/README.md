...
当前目录已经保留英文主文、英文补充材料、英文 title page、英文 cover letter 和英文 declarations。作者已统一为 Wenzhuo Wang 与通讯作者 Ying Shao，两人均使用 School of Science, Dalian Maritime University；Ying Shao 的邮箱暂留空。最终补充材料上传名将使用 `Online_Resource_1_supplementary_material.pdf`；当前 PDF、上传候选目录和 ZIP 按作者要求推迟到版本冻结后一次生成。生成式 AI 使用说明已按 BMB 要求写入方法部分。GitHub 公开版本已完成；最终系统上传前仍需补充通讯作者邮箱，并由两位作者确认 ORCID、Author Contributions 与软件许可。
...
- Wenzhuo Wang 和 Ying Shao 的 ORCID
- Ying Shao 的通讯作者邮箱
