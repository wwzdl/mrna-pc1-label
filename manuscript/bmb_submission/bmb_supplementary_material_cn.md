...
本节只补充主文 3.1 未展开的字段、参数和定义口径。源数据仍来自 Saluki 论文公开 supplement 中的 MOESM2 与 MOESM3 表格(Agarwal and Kelley 2022)。其中，MOESM2 是 human 与 mouse 的 transformed mRNA half-life gene × sample 矩阵，前两列为 Ensembl gene ID 和 gene name，后续列为不同 study、实验方法、细胞类型和重复样本的 transformed values；这是本文标签重建与留一研究影响分析的主输入。MOESM3 是 Saluki 发布的进一步处理矩阵，第三列为 “half-life (PC1)” summary label，后续列为 processed sample values。本文将 human 侧发布标签记为 Saluki human PC1，并将其固定为 fixed-target benchmark 的监督目标；将 mouse 侧发布 PC1 经 one-to-one ortholog 映射到 human genes 后作为外部小鼠协变量时，记为 Saluki mouse PC1。代码字段 `mouse_pc1` 则专指我们由 MOESM2 本地重建的 mouse prior。MOESM3 的 human 工作表说明 Saluki human PC1 基于剔除 Gejman 后的数据集，mouse 工作表基于全部 mouse 数据集。发布的 PC1 列因此用于定义固定监督目标和 Saluki 处理口径参照，processed sample columns 仅用于次级控制；二者均不参与从 MOESM2 开始的本地标签重建。由于 MOESM2 已经是 transformed values，本研究中的再分析不再重复执行 log 变换，而是从 sample-wise z-score 开始，再执行 iterative PCA 缺失值插补和 quantile normalization(Tipping and Bishop 1999; Troyanskaya et al. 2001; Bolstad et al. 2003)。
...
主文已报告本地重建标签与 Saluki 标签的总体一致性。在 full human 重建标签与 Saluki human PC1 共同覆盖的 13,921 个基因上，Pearson 为 0.948；mouse 对应值为 0.958。用于 Gejman 配对推断的固定 A2 集合还要求同时具有 no-Gejman 标签，因此包含 13,265 个基因，其 full-label baseline 为 0.9515。本文将这些至少 3 个观测的向量称为**审计阈值重建标签**；目标收缩改用**目标构建 human no-Gejman PC1**（至少 10 个观测）和**目标构建重建 mouse PC1**（至少 5 个观测）。覆盖阈值造成的差异很小但并非严格为零：两种 mouse PC1 在 11,882 个共同基因上的 Pearson=0.99907、z-scale RMSE=0.04312；两种 human no-Gejman PC1 在 12,644 个共同基因上的 Pearson=0.99827、z-scale RMSE=0.05878。因此，尽管其整体几何结构接近，正文仍使用不同名称。具体参数口径见补充表 S1，模型输入边界见补充表 S2。
...
| 审计阈值 coverage | `min_observed_per_gene=3` | 审计阈值 human/mouse PC1 vectors | 用于全量标签审计、留一研究影响分析，以及 fixed-target transfer 中的重建 prior |
| 目标构建 coverage | human `min_observed_per_gene=10`；mouse `min_observed_per_gene=5` | 目标构建 human no-Gejman 与重建 mouse PC1 vectors | 用于 0.10 同源基因信息目标收缩 |
...
| 0.10 同源基因信息目标收缩 | 目标构建 human no-Gejman PC1 + mapped 目标构建 mouse PC1 + $\lambda$ | 不是单独的新序列预测器 | human-dominant mammalian stability target | 不能写成新的直接 human half-life ground truth，也不能与固定目标结果混排 |
...
主分析的 leave-one-study-out estimator 对每个样本赋予相同权重。由于 Gejman 占 54 个 human 样本中的 15 个，删除该 study 比删除多数其他 study 移除更多样本。为量化这一影响，我们从其余 39 个非 Gejman 样本中无放回随机抽取 15 个样本，完整重跑标签重建流程，共重复 500 次；每个比较数据集都保留 Gejman。Gejman 的观测 stability 为 0.955；条件性删除分布中位数为 0.944，比较分布中央 95% 范围为 0.932-0.956，经验低尾比例为 0.964。因此，仅凭 PC1 几何变化不能把 Gejman 解释为经样本量校正后的异常 study。
...
**补充图 S4** Human study influence 的样本量与流程敏感性。a，在保留 Gejman 的同时删除 15 个非 Gejman 样本并完整重跑流程 500 次所得条件性 PC1 stability 比较分布；数值越低表示几何变化越大，Gejman 观测值在下尾并不极端。b，Saluki agreement 与 ortholog concordance 增益的条件性比较分布；橙色点为 Gejman 观测增益，均超过 500 次比较性删除结果。c，在 12 种 dynamic/fixed gene universe、coverage、rank 与 imputation 设置下，两条比较增益始终为正，且 Gejman 在 primary-style 设置中均排第一。d，采用两种 study-balanced estimator 后，几何排序均由第一变为第三。Panel source data 保存在 `manuscript/bmb_submission/figures/data/FigS04_panel_data.tsv`。

### 补充表 S4. Gejman study-influence 结果的样本数与流程敏感性

| 分析 | Gejman 结果 | 条件性比较 | 经验尾部比例或排序 | 解释 |
|:--|:--|:--|:--|:--|
| Primary sample-weighted geometry | stability 0.9550 | 中位数 0.9438；95% 区间 0.9319-0.9561 | 低尾比例=0.964 | 匹配删除样本数后，几何变化不极端 |
| Saluki agreement gain | +0.0314 | 中位数 -0.0926；95% 区间 -0.1151 至 -0.0713 | 高尾比例=1/501=0.002 | 条件性删除不能重现方向性增益 |
| Ortholog concordance gain | +0.0147 | 中位数 -0.0616；95% 区间 -0.0800 至 -0.0476 | 高尾比例=1/501=0.002 | 条件性删除不能重现跨物种增益 |
...
主文报告了最核心的 permutation 结果：真实双 prior global 模型的 Pearson 为 0.829，而 prior 被打乱后回到 0.745。这里最关键的是打乱对象。第一，在每个 outer fold 内，fit、inner-validation 和 held-out 三个分区分别独立打乱，因此不会从训练分区恢复 held-out gene-prior 对应。第二，打乱只发生在原本有 prior 数值的基因之间；哪些基因没有 prior、对应的可用性 0/1 指示列、以及 high-confidence 0/1 指示列都保持不变，因此缺失结构本身没有被破坏。第三，真实 prior 相对 shuffled prior 的 Pearson 提升约为 +0.085，R2 提升约为 +0.135，说明起作用的是 prior 数值与基因身份之间的对应关系，而不是额外的特征列。

在三组独立随机划分的 10-fold OOF 评估中，真实双 prior global 模型为 Pearson 0.830±0.001，shuffled-prior control 为 0.748±0.001，human-only `compact_all` 也为 0.748±0.001；coverage-aware fallback 为 0.832±0.001。这里的 ± 是 across-seed SD。将每个 gene 的三组 seed-specific OOF predictions 求均值得到 seed-averaged OOF ensemble。在以该已拟合 ensemble 和 gene resampling 为条件的 paired bootstrap 中，real prior 相对 shuffled prior 的增益为 0.0794（95% CI 0.0735-0.0855），相对 human-only 的增益为 0.0799（95% CI 0.0740-0.0862）。这些区间不包含重新训练或重新划分 folds 的不确定性。
...
具体实现如下。第一，只保留同时具有 `mouse_pc1` 与 Saluki mouse PC1 的 `both_priors_available` genes（N=11,107），因为 prior-only 分解需要每个 gene 同时具备两类 mouse-side prior。第二，在每个 5-fold outer split 内，再执行 5-fold inner cross-fitting：RidgeCV 每次在四个 inner folds 上拟合并预测第五个 inner fold，从而为每个 outer-training gene 生成未在其自身 target 上拟合的 nuisance-prior prediction；用 Saluki human PC1 减去该 inner-OOF prediction 定义 residual target。随后另用全部 outer-training genes 拟合 RidgeCV，预测 outer held-out genes。第三，XGBoost/CUDA 只用 outer-training human `compact_all` features 与 cross-fitted residuals 训练。第四，outer-held-out genes 的最终预测为 held-out prior prediction 与 held-out compact-residual prediction 之和。Outer-held-out human target 不进入任一训练阶段。
...
| Prior 加 human residual | 11,107 | 2 | 1,802 | 0.826 | 0.817 | 0.675 | 0.048 | 12.85% |
...
补充式（S2）表明，在 prior-only model 已经解释的部分之外，human `compact_all` features 还能解释约 12.9% 的剩余方差。因此，prior-enhanced model 更准确地说是 strong mouse prior 加 human correction，而不是 prior 单独就足够。
...
该分析改变的是**监督目标**，global prior benchmark 改变的是**输入信息**。0.10 同源基因信息收缩目标由 90% 目标构建 human no-Gejman PC1（至少 10 个观测）和 10% mapped 目标构建重建 mouse PC1（至少 5 个观测）在各自 z-score 后组成；没有 ortholog signal 的 human genes 保留 human label。本文将其解释为从 multi-study compendium 得到的 human-dominant mammalian stability score。

几何证据支持 human dominance。在 12,307 个 model-eligible genes 上，收缩 target 与目标构建 human no-Gejman PC1 的 Pearson 为 0.9982（95% bootstrap CI 0.9981-0.9983）。在单独定义的 10,768-pair mapped set 中，收缩前目标构建 human no-Gejman PC1 与目标构建重建 mouse PC1 的 Pearson 为 0.789，RMSE 为 0.663，MAE 为 0.512；收缩后 target 与 human label 的 Pearson 为 0.9979（RMSE=0.065；MAE=0.050），与 mouse label 的 Pearson 为 0.827。其 label shift 也远小于 ordinary study-to-study differences（S9 和补充表 S13）。
...
Cross-target evaluation 进一步检查“改 target 是否损害原 human 标签”。在同一 12,307-gene universe、同一 1802 human-only features、同一 10-fold × 3 seeds 和相同模型参数下，以 human no-Gejman PC1 训练后在该目标上评估得到 r=0.7476±0.0010；改用 shrinkage target 训练、仍在原 human target 上评估得到 r=0.7480±0.0010。Seed-averaged OOF ensemble 的 paired difference 为 +0.0003（95% CI -0.0006 至 0.0012），区间以已拟合 predictions 和 gene resampling 为条件。该区间跨过 0，因此未检测到下降；但由于没有预先设定等效界值，不能据此宣称形式等效。

使用显式 mouse priors 预测 shrinkage target 时，主要设定达到 0.8551±0.0007；同一宇宙上 fixed Saluki human PC1 的 prior-enhanced baseline 为 0.8394±0.0007。主要 shrinkage-target predictor 使用审计阈值重建 mouse PC1 与 Saluki mouse PC1。为直接检查模型是否读取了构成 target 的 10% mouse 向量，严格成对分析将本地 prior 换成精确的目标构建重建 mouse PC1：该精确双 prior 设定为 0.8537±0.0004；移除这条精确向量、但保留 Saluki mouse PC1 和全部 availability/confidence indicators 后为 0.8524±0.0005。精确直接向量只增加 0.0013，而仅有 indicators、没有 mouse values 时为 0.7538±0.0007。补充表 S11 汇总全部设定。该结果把很小的直接 target-vector 贡献与外部跨物种协变量携带的主要信号区分开。
...
| --- | --- | ---: | --- | --- |
| L1 | 审计阈值 human full reconstruction | 16,444 genes | MOESM2 human genes 在 54 个样本中至少 3 个有观测 | Full-human PC1、sample PCA 和 human LOO 的起始标签 |
| L2 | 审计阈值 mouse full reconstruction | 16,951 genes | MOESM2 mouse genes 在 27 个样本中至少 3 个有观测 | Mouse PC1、mouse PCA/LOO 和 fixed-target reconstructed mouse prior 来源 |
| L3 | 审计阈值 human no-Gejman reconstruction | 15,788 genes | Human genes 在 39 个 non-Gejman 样本中至少 3 个有观测 | No-Gejman PC1 和 primary dynamic-coverage Gejman comparison |
...
| S1 | 目标构建 human no-Gejman universe | 12,644 genes | Human no-Gejman genes 至少 10 个观测；mouse comparator 至少 5 个观测 | 构建 0.10 ortholog-informed target |
...
|:--|--:|--:|--:|--:|:--|:--|:--|:--|
| 0.10 收缩目标；目标构建 mouse PC1 | 0.10 | 12,307 | 0.987 | 0.827 | 0.855 ± 0.001 | 0.754 ± 0.001 | 0.753 ± 0.001 | 0.102 ± 0.001 |
...
|:--|--:|--:|--:|:--|:--|:--|
| 仅 human features | 1802 | 10 | 3 | 0.7538 ± 0.0010 | 0.7533 ± 0.0013 | -0.0857 ± 0.0010 |
| 仅 availability/confidence indicators | 1806 | 10 | 3 | 0.7538 ± 0.0007 | 0.7533 ± 0.0009 | -0.0857 ± 0.0007 |
| 仅 Saluki mouse PC1 | 1805 | 10 | 3 | 0.8521 ± 0.0005 | 0.8347 ± 0.0006 | 0.0127 ± 0.0005 |
| 严格 no-direct：Saluki value 加全部 indicators | 1807 | 10 | 3 | 0.8524 ± 0.0005 | 0.8351 ± 0.0005 | 0.0130 ± 0.0005 |
| 仅精确目标构建 mouse PC1 | 1805 | 10 | 3 | 0.8525 ± 0.0003 | 0.8341 ± 0.0004 | 0.0130 ± 0.0003 |
| 精确目标构建 mouse PC1 加 Saluki mouse PC1 | 1808 | 10 | 3 | 0.8537 ± 0.0004 | 0.8354 ± 0.0004 | 0.0143 ± 0.0004 |
| 主要审计阈值 mouse PC1 加 Saluki mouse PC1 | 1808 | 10 | 3 | 0.8551 ± 0.0007 | 0.8366 ± 0.0006 | 0.0156 ± 0.0007 |

最后一行复现正文主要 target-prediction 输入。严格 direct-vector 对比为第六行减第四行：在保持 Saluki mouse PC1 与全部 indicators 不变时，加入精确目标构建 mouse PC1 使 target correlation 增加 0.0013。

### 补充表 S12. Human-mouse ortholog label distance

| 比较 | Ortholog 子集 | N | Pearson r | Spearman ρ | MSE | RMSE | MAE | 绝对误差中位数 | 绝对误差第 95 百分位数 |
|:--|:--|--:|--:|--:|--:|--:|--:|--:|--:|
| Human no-Gejman PC1 vs 目标构建 mouse PC1 | 全部 one-to-one | 10,768 | 0.789 | 0.782 | 0.440 | 0.663 | 0.512 | 0.413 | 1.330 |
| Shrinkage target vs 目标构建 mouse PC1 | 全部 one-to-one | 10,768 | 0.827 | 0.820 | 0.361 | 0.601 | 0.464 | 0.373 | 1.209 |
...
| Human no-Gejman PC1 vs 目标构建 mouse PC1 | 高置信 | 10,626 | 0.791 | 0.784 | 0.436 | 0.660 | 0.510 | 0.410 | 1.326 |
| Shrinkage target vs 目标构建 mouse PC1 | 高置信 | 10,626 | 0.829 | 0.822 | 0.357 | 0.598 | 0.462 | 0.371 | 1.201 |
...
| 0.10 同源基因信息收缩目标 | human no-Gejman PC1 | 12,307 | 0.7480 ± 0.0010 | 0.5582 ± 0.0014 | 0.6680 ± 0.0011 | 0.5248 ± 0.0009 |
| human no-Gejman PC1 | 0.10 同源基因信息收缩目标 | 12,307 | 0.7532 ± 0.0010 | 0.5651 ± 0.0015 | 0.6627 ± 0.0012 | 0.5215 ± 0.0014 |
| 0.10 同源基因信息收缩目标 | 0.10 同源基因信息收缩目标 | 12,307 | 0.7538 ± 0.0010 | 0.5664 ± 0.0014 | 0.6617 ± 0.0010 | 0.5201 ± 0.0009 |
| human no-Gejman PC1 | Saluki human PC1 | 12,307 | 0.7529 ± 0.0014 | 0.5638 ± 0.0021 | 0.6723 ± 0.0016 | 0.5273 ± 0.0017 |
| 0.10 同源基因信息收缩目标 | Saluki human PC1 | 12,307 | 0.7533 ± 0.0013 | 0.5650 ± 0.0019 | 0.6714 ± 0.0015 | 0.5261 ± 0.0014 |

在原 human no-Gejman PC1 上，seed-averaged OOF ensemble 的 Pearson 分别为 0.75233（human-target training）和 0.75259（shrinkage-target training）。Paired difference 为 +0.00026，2,000 次 gene bootstrap 的 95% CI 为 -0.00061 至 0.00118。该区间以已拟合的三 seed ensemble 与 gene resampling 为条件，不代表重新训练或 split uncertainty。置信区间跨越 0，说明当前分辨率下没有证据表明目标收缩训练降低或提高了原 human 标签的可预测性；本文不把这一结果表述为未经预设界值的统计等效性检验。

### 补充表 S15. 关键相关差值的 paired bootstrap uncertainty

除 cross-target analysis 单独使用 2,000 次 bootstrap 外，本表使用 5,000 次 gene 或 one-to-one ortholog-pair bootstrap。Prior rows 中的 prediction 是三组 10-fold OOF vectors 的 gene-wise mean，即 seed-averaged OOF ensemble。区间以已拟合 predictions 与重采样 genes/pairs 为条件，不包括重新训练不确定性。重采样以 gene 或 ortholog pair 为单位，没有显式建模 pathway 或 paralog family 内的相关性。

本文所有 prediction folds 均为 gene-level 随机划分，而非按 paralog family 分组。报告的 OOF 数值因此估计的是该分折规则下对 held-out genes 的泛化能力；gene-family-held-out 泛化仍未检验。
...
| 类别 | 路径或入口 | 用途 |
|:--|:--|:--|
| 版本化仓库 | `https://github.com/wwzdl/mrna-pc1-label` | 提供代码、结果表、图件脚本和环境说明；当前两作者稿件审计标签为 `mRNA-PC1-label-v1.4.3` |
...
| XGBoost 参数来源 | `scripts/reproduce_xgb_parameter_scan.py`、`results/human_xgboost_gpu_saluki_refine.tsv`、`results/human_xgboost_parameter_provenance.json` | 复现同一 compendium 上的 5-setting 初步扫描，并记录冻结的选择规则与解释限制 |
...
| 0.10 同源基因信息目标收缩 | `results/ortholog_regularized_label_10fold_main/summary_by_label.tsv`、`results/ortholog_regularized_label_multiseed/summary_by_label.tsv`、`results/ortholog_prior_ablation_lambda0p1_10fold/summary_by_setting.tsv` | 记录 $\lambda = 0.10$ target benchmark、λ sensitivity 与 prior ablation |
