...
- 完整 12,307-gene target universe：target vs human no-Gejman PC1 `r = 0.9982`（95% CI `0.9981-0.9983`）
...
- BMB DOCX-only 自动审计：`0 failure / 1 warning / 158 checks`；唯一 warning 为当前打开的英文主文与补充材料产生的 WPS 临时锁文件
- English abstract：`232 words`（目标 `150-250 words`）
- keywords：`6`
...
GitHub 仓库 [wwzdl/mrna-pc1-label](https://github.com/wwzdl/mrna-pc1-label) 已公开，Data/Code Availability 已改为当前时态；本轮预投稿审计版本标记为 `mRNA-PC1-label-v1.3`。
