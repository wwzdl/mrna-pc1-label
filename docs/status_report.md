...
1. **Study-aware label audit**
   ...
   Leave-one-study-out PC1 stability 在不使用 Saluki label 或下游模型分数的情况下，将 `Gejman` 识别为 human compendium 中的主导影响来源。Saluki human PC1 agreement 和 human-mouse ortholog concordance 用于后续验证，而非筛选。
...
3. **Weak ortholog regularization at `lambda = 0.10`**

   `0.10 ortholog-regularized target` 是一个独立核心标签构建结果，而非辅助调参。它与 human no-Gejman PC1 几乎重合，其偏移远小于普通 study-level variability；cross-target evaluation 未检出原 human label 可预测性下降。该目标是 human-dominant mammalian stability target，不是 pure human ground truth 或 Saluki human PC1 的替代排名。
...
- Cross-species transfer 改变输入，ortholog regularization 改变 target，两类数值不构成同一 leaderboard。
...
- BMB DOCX-only 冻结前自动审计：`136 checks, 0 failures, 0 warnings`
- English abstract：`238 words`
- keywords：`6`
...
- 最终源码冻结后的完整项目备份固定为 `../workspace_archive/mrna-pc1-label_paper_pca_presubmission_20260714.tar.zst`，其 SHA-256 保存在同名 `.sha256` 边车文件；完整性结果在项目源码冻结后记录。
...
- Leave-one-study-out influence 同时包含 sample count 和 profile direction 差异；没有 size-matched null 时，`Gejman` 只能称为主导影响来源，不能称为经样本数校正的统计异常值。
